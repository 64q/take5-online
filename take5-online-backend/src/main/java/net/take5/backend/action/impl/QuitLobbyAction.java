...
public class QuitLobbyAction extends AbstractAction<QuitLobbyParams, QuitLobbyResponse> implements MessageSourceAware
{
    ...
    private void notifyQuitLobby(Set<User> users)
    {
        for (User user : users) {
            ...
            notification.setAction(OutputAction.NOTIFICATION);
            notification.setNotification(messageSource.getMessage(MessageKey.NOTIFICATION_REMOVED_FROM_LOBBY, null,
                    Locale.getDefault()));

            try {
                user.getSession().getBasicRemote().sendObject(notification);
            } catch (IOException | EncodeException e) {
                LOG.error("Erreur lors de la sérialisation du message d'envoi de notification de suppression du lobby",
                        e);
            }
        }
    }
